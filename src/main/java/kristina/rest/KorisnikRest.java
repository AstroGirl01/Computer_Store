...
public Response updateKorisnik(Korisnik korisnik) {
    try (Connection con = ResourcesManager.getConnection()) {
        KorisnikDao.getInstance().updateAllFields(korisnik.getUsername(), korisnik, con);
...
@Path("/{username}")
@Produces(MediaType.TEXT_PLAIN)
public Response deleteKorisnik(@PathParam("username") String username) {
    try {
        korisnikService.deleteKorisnik(username);
        return Response.ok("Korisnik '" + username + "' je uspešno obrisan.").build();
    } catch (prodavnica_exception e) {
        ...
        return Response.status(Response.Status.INTERNAL_SERVER_ERROR)
                       .entity("Greška prilikom brisanja korisnika: " + e.getMessage())
                       .build();
    }
}
